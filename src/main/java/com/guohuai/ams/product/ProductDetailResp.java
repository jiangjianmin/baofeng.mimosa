...
public class ProductDetailResp extends BaseResp {
	...
	public ProductDetailResp(Product p,String productEleOid,String productIntroOid,String productActivityOid,String productElementTitle,String productIntroTitle,String activityDetailTitle) {
		this.oid = p.getOid();
		this.code = p.getCode();
		this.name = p.getName();
		...
		if (p.getProductPackage() != null) {
			this.packageOid = p.getProductPackage().getOid();
			this.packageName = p.getProductPackage().getName();
			this.packageStr = "产品包（" + p.getProductPackage().getName() + "）";
		}else{
			this.packageStr = "产品";
		}
		/**循环开放相关**/
		this.movedupRedeemLockDays=p.getMovedupRedeemLockDays();
		this.movedupRedeemMinPay=p.getMovedupRedeemMinPay();
		this.movedupRedeemRate=p.getMovedupRedeemRate();

		this.repayInterestStatus = p.getRepayInterestStatus(); //派息状态
		this.repayLoanStatus = p.getRepayLoanStatus(); //还本付息状态
		...
		this.ifP2PStr = p.getIfP2P()== 1?"是":"否";//是否为P2P产品
	...
	/** 产品包新加   **/
	private String packageOid; //产品包ID
	private String packageName; //产品包名称
	private String packageStr;
	/** 产品包新加   **/
	
	/** 派息审核新加   **/
	private String repayInterestStatus; //派息状态
	private String repayLoanStatus; //还本付息状态
	/** 派息审核新加   **/
	...
	private String useRaise;
	private Integer useRaiseId;//是否可以使用加息券枚举项
	/**
	 * 关联产品详情
	 */
	private String productElementStr;//产品要素的ID
	private String productIntroStr;//产品说明ID
	private String activityDetailStr;//活动产品详情介绍ID
