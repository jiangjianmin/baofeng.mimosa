...
public class WyHttpClientUtil {
    ...
    public String postSSLUrlWithParams(String url, Map params, String keyStorePath) {
        return postSSLUrlWithParams(url,params,CHARSET,keyStorePath);
    }
     ...
     * @return
     */
    public String postSSLUrlWithParams(String url, Map params, String encoding, String keyStorePath) {
        String encode = CHARSET;
        if (!StringUtils.isEmpty(encoding)) {
            encode = encoding;
        }
      
        log.info("HttpClient调用方式");
        DefaultHttpClient httpclient = new DefaultHttpClient();
        HttpPost httpost = new HttpPost(url);
        // 添加参数
        List<NameValuePair> nvps = new ArrayList<NameValuePair>();
