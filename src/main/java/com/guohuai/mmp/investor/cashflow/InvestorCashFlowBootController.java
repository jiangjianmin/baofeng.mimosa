...
public class InvestorCashFlowBootController extends BaseController {
	...
	@ResponseBody
	public ResponseEntity<PagesRep<InvestorCashFlowQueryRep>> query(HttpServletRequest request,
			@And({@Spec(params = "tradeType", path = "tradeType", spec = In.class),
				...
				@Spec(params = "tradeOrderisAuto", path = "tradeOrder.isAuto", spec = Equal.class),
		...
		PagesRep<InvestorCashFlowQueryRep> rep = this.investorCashFlowService.query(spec, pageable);
		
		return new ResponseEntity<PagesRep<InvestorCashFlowQueryRep>>(rep, HttpStatus.OK);
	}
