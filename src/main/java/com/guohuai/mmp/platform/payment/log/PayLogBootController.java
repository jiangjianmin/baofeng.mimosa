...
public class PayLogBootController extends BaseController {
	...
	@ResponseBody
	public ResponseEntity<PagesRep<PayLogQueryRep>> mng(HttpServletRequest request,
			@And({@Spec(params = "interfaceName", path = "interfaceName", spec = Equal.class)}) Specification<PayLogEntity> spec,
		...
		PagesRep<PayLogQueryRep> rep = this.payLogService.mng(spec, pageable);
		return new ResponseEntity<PagesRep<PayLogQueryRep>>(rep, HttpStatus.OK);
	}
